  ...
  module CLI
    ...
    def get_or_create_label(service, label_name, label_cache)
      ...
      unless label_cache.key?(full_name)
        label_id = GmailSorter.create_label(service, full_name)
    ...
    def run(argv = ARGV)
      ...
      begin
        service = GmailSorter.build_service(CREDENTIALS_PATH, TOKEN_PATH)
      rescue Errno::ENOENT
      ...
      emails = GmailSorter.fetch_unread_emails(service, options[:count])
      if emails.empty?
      ...
      label_cache = GmailSorter.list_labels(service)
      rows = []
      labels_created = 0
      labels_reused = 0
      ...
      emails.each do |email|
        ...
        category = GmailSorter.categorize(email, model: options[:model], categories: DEFAULT_CATEGORIES)
        ...
        if options[:dry_run]
          ...
          GmailSorter.apply_label(service, email[:id], label_id)
          if created
            labels_created += 1
          else
            labels_reused += 1
